...
type MoacRPCTestSuite struct {
	suite.Suite
	rpc *MoacRPC
}

func (s *MoacRPCTestSuite) registerResponse(result string, callback func([]byte)) {
	httpmock.Reset()
...
func (s *MoacRPCTestSuite) registerResponseError(err error) {
	httpmock.Reset()
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
		return nil, err
	})
...
func (s *MoacRPCTestSuite) getBody(request *http.Request) []byte {
...
func (s *MoacRPCTestSuite) methodEqual(body []byte, expected string) {
	value := gjson.GetBytes(body, "method").String()

	s.Require().Equal(expected, value)
}

func (s *MoacRPCTestSuite) paramsEqual(body []byte, expected string) {
...
func (s *MoacRPCTestSuite) SetupSuite() {
	s.rpc = NewMoacRPC("http://127.0.0.1:8545", WithHttpClient(http.DefaultClient), WithLogger(nil), WithDebug(false))

	httpmock.Activate()
}

func (s *MoacRPCTestSuite) TearDownSuite() {
	httpmock.Deactivate()
}

func (s *MoacRPCTestSuite) TearDownTest() {
	httpmock.Reset()
}

func (s *MoacRPCTestSuite) TestChain3ClientVersion() {
	response := `{"jsonrpc":"2.0", "id":1, "result": "test client"}`

	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
		body := s.getBody(request)
		s.methodEqual(body, "chain3_clientVersion")
		s.paramsEqual(body, `null`)

		return httpmock.NewStringResponse(200, response), nil
	})

	v, err := s.rpc.Chain3ClientVersion()
	s.Require().Nil(err)
	s.Require().Equal("test client", v)
}

func (s *MoacRPCTestSuite) TestCall() {
	// Test http error
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
...
func (s *MoacRPCTestSuite) TestChain3Sha3() {
	response := `{"jsonrpc":"2.0", "id":1, "result": "sha3result"}`

	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
		body := s.getBody(request)
		s.methodEqual(body, "chain3_sha3")
		s.paramsEqual(body, `["0x64617461"]`)

		return httpmock.NewStringResponse(200, response), nil
	})

	result, err := s.rpc.Chain3Sha3([]byte("data"))
	s.Require().Nil(err)
	s.Require().Equal("sha3result", result)
}

func (s *MoacRPCTestSuite) TestNetVersion() {
...
func (s *MoacRPCTestSuite) TestNetListening() {
...
func (s *MoacRPCTestSuite) TestNetPeerCount() {
	// Test error
...
func (s *MoacRPCTestSuite) TestMoacProtocolVersion() {
	s.registerResponse(`"54"`, func(body []byte) {
		s.methodEqual(body, "mc_protocolVersion")
		s.paramsEqual(body, "null")
	})

	protocolVersion, err := s.rpc.MoacProtocolVersion()
	s.Require().Nil(err)
	s.Require().Equal("54", protocolVersion)
}

func (s *MoacRPCTestSuite) TestMoacSyncing() {
	s.registerResponseError(errors.New("Error"))
	syncing, err := s.rpc.MoacSyncing()
	s.Require().NotNil(err)
...
func (s *MoacRPCTestSuite) TestMoacCoinbase() {
	s.registerResponse(`"0x407d73d8a49eeb85d32cf465507dd71d507100c1"`, func(body []byte) {
		s.methodEqual(body, "mc_coinbase")
		s.paramsEqual(body, "null")
	})

	address, err := s.rpc.MoacCoinbase()
	s.Require().Nil(err)
	s.Require().Equal("0x407d73d8a49eeb85d32cf465507dd71d507100c1", address)
}
func (s *MoacRPCTestSuite) TestMoacMining() {
	s.registerResponse(`true`, func(body []byte) {
		s.methodEqual(body, "mc_mining")
		s.paramsEqual(body, "null")
	})

	mining, err := s.rpc.MoacMining()
	s.Require().Nil(err)
...
func (s *MoacRPCTestSuite) TestMoacHashrate() {
	s.registerResponseError(errors.New("Error"))
	hashrate, err := s.rpc.MoacHashrate()
	s.Require().NotNil(err)

	s.registerResponse(`"0x38a"`, func(body []byte) {
		s.methodEqual(body, "mc_hashrate")
		s.paramsEqual(body, "null")
	})
...
func (s *MoacRPCTestSuite) TestMoacGasPrice() {
	s.registerResponseError(errors.New("Error"))
	gasPrice, err := s.rpc.MoacGasPrice()
	s.Require().NotNil(err)

	s.registerResponse(`"0x09184e72a000"`, func(body []byte) {
		s.methodEqual(body, "mc_gasPrice")
		s.paramsEqual(body, "null")
	})
...
func (s *MoacRPCTestSuite) TestMoacAccounts() {
	s.registerResponse(`["0x407d73d8a49eeb85d32cf465507dd71d507100c1"]`, func(body []byte) {
		s.methodEqual(body, "mc_accounts")
		s.paramsEqual(body, "null")
	})

	accounts, err := s.rpc.MoacAccounts()
	s.Require().Nil(err)
	s.Require().Equal([]string{"0x407d73d8a49eeb85d32cf465507dd71d507100c1"}, accounts)
}

func (s *MoacRPCTestSuite) TestMoacBlockNumber() {
	s.registerResponseError(errors.New("Error"))
	blockBumber, err := s.rpc.MoacBlockNumber()
	s.Require().NotNil(err)

	s.registerResponse(`"0x37eb38"`, func(body []byte) {
		s.methodEqual(body, "mc_blockNumber")
		s.paramsEqual(body, "null")
	})
...
func (s *MoacRPCTestSuite) TestMoacGetBalance() {
	address := "0x407d73d8a49eeb85d32cf465507dd71d507100c1"
	s.registerResponseError(errors.New("Error"))
	balance, err := s.rpc.MoacGetBalance(address, "latest")
	s.Require().NotNil(err)

	s.registerResponse(`"0x486d06b0d08d05909c4"`, func(body []byte) {
		s.methodEqual(body, "mc_getBalance")
...
func (s *MoacRPCTestSuite) TestMoacGetStorageAt() {
	...
	s.registerResponse(`"0x00000000000000000000000000000000000000000000000000000000000004d2"`, func(body []byte) {
		s.methodEqual(body, "mc_getStorageAt")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "0x21", "pending"]`, data))
	})

	result, err := s.rpc.MoacGetStorageAt(data, position, tag)
	s.Require().Nil(err)
	s.Require().Equal("0x00000000000000000000000000000000000000000000000000000000000004d2", result)
}

func (s *MoacRPCTestSuite) TestMoacGetTransactionCount() {
	address := "0x407d73d8a49eeb85d32cf465507dd71d507100c1"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.MoacGetTransactionCount(address, "latest")
	s.Require().NotNil(err)

	s.registerResponse(`"0x10"`, func(body []byte) {
		s.methodEqual(body, "mc_getTransactionCount")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "latest"]`, address))
	})
...
func (s *MoacRPCTestSuite) TestMoacGetBlockTransactionCountByHash() {
	hash := "0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.MoacGetBlockTransactionCountByHash(hash)
	s.Require().NotNil(err)

	s.registerResponse(`"0xb"`, func(body []byte) {
		s.methodEqual(body, "mc_getBlockTransactionCountByHash")
...
func (s *MoacRPCTestSuite) TestMoacGetBlockTransactionCountByNumber() {
	number := 2384732
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.MoacGetBlockTransactionCountByNumber(number)
	s.Require().NotNil(err)

	s.registerResponse(`"0xe8"`, func(body []byte) {
		s.methodEqual(body, "mc_getBlockTransactionCountByNumber")
...
func (s *MoacRPCTestSuite) TestMoacGetUncleCountByBlockHash() {
	hash := "0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.MoacGetUncleCountByBlockHash(hash)
	s.Require().NotNil(err)

	s.registerResponse(`"0xa"`, func(body []byte) {
		s.methodEqual(body, "mc_getUncleCountByBlockHash")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, hash))
	})
...
func (s *MoacRPCTestSuite) TestMoacGetUncleCountByBlockNumber() {
	number := 3987434
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.MoacGetUncleCountByBlockNumber(number)
	s.Require().NotNil(err)

	s.registerResponse(`"0x386"`, func(body []byte) {
		s.methodEqual(body, "mc_getUncleCountByBlockNumber")
...
func (s *MoacRPCTestSuite) TestSendTransaction() {
	...
	result := "0xea1115eb5"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "mc_sendTransaction")
		s.paramsEqual(body, `[{
			"from": "0x3cc1a3c082944b9dba70e490e481dd56",
			"to": "0x1bf21cb1dc384d019a885a06973f7308",
			"gas": "0x6144",
	...
	txid, err := s.rpc.MoacSendTransaction(t)
	s.Require().Nil(err)
...
func (s *MoacRPCTestSuite) TestMoacSendRawTransaction() {
	data := "0xd46e8dd67c5d32be8d46e8dd67c5d32be8058bb8eb970870f072445675058bb8eb970870f072445675"
	result := "0xe670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d1527331"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "mc_sendRawTransaction")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, data))
	})

	txid, err := s.rpc.MoacSendRawTransaction(data)
	s.Require().Nil(err)
	s.Require().Equal(result, txid)
}

func (s *MoacRPCTestSuite) TestMoacGetCompilers() {
	s.registerResponse(`["solidity", "some comp"]`, func(body []byte) {
		s.methodEqual(body, "mc_getCompilers")
		s.paramsEqual(body, "null")
	})

	compilers, err := s.rpc.MoacGetCompilers()
	s.Require().Nil(err)
	s.Require().Equal([]string{"solidity", "some comp"}, compilers)

}

func (s *MoacRPCTestSuite) TestGetBlock() {
	s.registerResponseError(errors.New("Error"))
	block, err := s.rpc.getBlock("mc_getBlockByHash", true)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "mc_getBlockByHash")
	})
	...
	block, err = s.rpc.getBlock("mc_getBlockByHash", false)
	s.Require().Nil(err)
	s.Require().NotNil(block)
	s.Require().Equal(hash, block.Hash)
...
func (s *MoacRPCTestSuite) TestMoacGetBlockByHash() {
	// Test with transactions
	hash := "0x111"
	...
	_, err := s.rpc.MoacGetBlockByHash(hash, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.MoacGetBlockByHash(hash, false)
	s.Require().Nil(err)
}

func (s *MoacRPCTestSuite) TestMoacGetBlockByNumber() {
	// Test with transactions
	number := 3274863
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "mc_getBlockByNumber")
		s.paramsEqual(body, `["0x31f86f", true]`)
	})

	_, err := s.rpc.MoacGetBlockByNumber(number, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.MoacGetBlockByNumber(number, false)
	s.Require().Nil(err)
}

func (s *MoacRPCTestSuite) TestMoacCall() {
	s.registerResponse(`"0x11"`, func(body []byte) {
		s.methodEqual(body, "mc_call")
		s.paramsEqual(body, `[{"from":"0x111","to":"0x222"}, "ttt"]`)
	})

	result, err := s.rpc.MoacCall(T{
		From: "0x111",
		To:   "0x222",
	}, "ttt")
	s.Require().Nil(err)
...
func (s *MoacRPCTestSuite) TestMoacEstimateGas() {
	s.registerResponseError(errors.New("error"))
	result, err := s.rpc.MoacEstimateGas(T{
		From: "0x111",
		To:   "0x222",
	})
	s.Require().NotNil(err)
	...
	s.registerResponse(`"0x5022"`, func(body []byte) {
		s.methodEqual(body, "mc_estimateGas")
...
func (s *MoacRPCTestSuite) TestMoacGetTransactionReceipt() {
	hash := "0x9c17afa5336d3cfd47e2e795520959b92e627e123e538fd4d5d7ece9025a8dce"
	s.registerResponseError(errors.New("error"))
	receipt, err := s.rpc.MoacGetTransactionReceipt(hash)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "mc_getTransactionReceipt")
...
func (s *MoacRPCTestSuite) TestGetTransaction() {
	result := `{
...
func (s *MoacRPCTestSuite) TestMoacGetTransactionByHash() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "mc_getTransactionByHash")
		s.paramsEqual(body, `["0x123"]`)
	})

	t, err := s.rpc.MoacGetTransactionByHash("0x123")
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *MoacRPCTestSuite) TestMoacGetTransactionByBlockHashAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "mc_getTransactionByBlockHashAndIndex")
		s.paramsEqual(body, `["0x623", "0x12"]`)
	})

	t, err := s.rpc.MoacGetTransactionByBlockHashAndIndex("0x623", 18)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *MoacRPCTestSuite) TestMoacGetTransactionByBlockNumberAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "mc_getTransactionByBlockNumberAndIndex")
		s.paramsEqual(body, `["0x1f537da", "0xa"]`)
	})

	t, err := s.rpc.MoacGetTransactionByBlockNumberAndIndex(32847834, 10)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *MoacRPCTestSuite) TestMoacNewFilterWithAddress() {
	...
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "mc_newFilter")
		s.paramsEqual(body, fmt.Sprintf(`[{"address": ["%s"]}]`, address[0]))
	})

	filterID, err := s.rpc.MoacNewFilter(filterData)
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *MoacRPCTestSuite) TestMoacNewFilterWithTopics() {
	topics := [][]string{
		{
...
func (s *MoacRPCTestSuite) TestMoacNewFilterWithAddressAndTopics() {
	topics := [][]string{
		{"0xb2b2eeeee341e560da3d439ef5e5309d78a22a66"},
		{"0xb2b2fffff341e560da3d439ef5e5309d78a22a66"},
	}
...
func (s *MoacRPCTestSuite) TestMoacNewBlockFilter() {
	result := "0x6996a3a4788d4f2067108d1f536d4330"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "mc_newBlockFilter")
	})

	filterID, err := s.rpc.MoacNewBlockFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *MoacRPCTestSuite) TestMoacNewPendingTransactionFilter() {
	result := "0x153"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "mc_newPendingTransactionFilter")
	})

	filterID, err := s.rpc.MoacNewPendingTransactionFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *MoacRPCTestSuite) TestMoacGetFilterChanges() {
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "mc_getFilterChanges")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.MoacGetFilterChanges(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *MoacRPCTestSuite) TestMoacGetFilterLogs() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := `[{
		"address": "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
		"blockHash": "0x9d9838090bb7f6194f62acea788688435b79cc44c62dcf1479abd9f2c72a7d5c",
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "mc_getFilterLogs")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.MoacGetFilterLogs(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *MoacRPCTestSuite) TestMoacGetLogs() {
	params := FilterParams{
		FromBlock: "0x1",
		ToBlock:   "0x10",
	...
	logs, err := s.rpc.MoacGetLogs(params)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *MoacRPCTestSuite) TestMoacUninstallFilter() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := "true"
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "mc_uninstallFilter")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	uninstall, err := s.rpc.MoacUninstallFilter(filterID)
	s.Require().Nil(err)
	boolRes, _ := strconv.ParseBool(result)
	s.Require().Equal(boolRes, uninstall)
}

func TestMoacRPCTestSuite(t *testing.T) {
	suite.Run(t, new(MoacRPCTestSuite))
}

func TestMoacError(t *testing.T) {
	var err error
	err = MoacError{-32555, "Messg"}
	require.Equal(t, "Error -32555 (Messg)", err.Error())

	err = MoacError{32847, "Kuku"}
	require.Equal(t, "Error 32847 (Kuku)", err.Error())
}

func TestMoac1(t *testing.T) {
	client := NewMoacRPC("")
	require.Equal(t, int64(1000000000000000000), Moac1().Int64())
	require.Equal(t, int64(1000000000000000000), client.Moac1().Int64())
}
